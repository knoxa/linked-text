...
intent of those concepts are the various names of that person. I express these hypotheses as a new context where the objects are individual people and the attributes are names.
...
These assumptions are not valid in every case, but do produce some useful results that can be developed further. In general, the **normal** agent is less likely to put together two names that belong to different people, but will falsely separate names with slight variations in spelling. The two phonetic agents cope better with variations in spelling, at the cost of sometimes putting the names of two different people together. The **soundex** and **metaphone** agents may or may not agree. Both make mistakes. 

The hypotheses that the three agents generate are [normal.xml](results/hypothesis/normal.xml), [soundex.xml](results/hypothesis/soundex.xml) and [metaphone.xml](results/hypothesis/metaphone.xml).
...
I can merge these with hypotheses generated by other agents, and use them to influence the assessment of those hypotheses. Some experimentation needed here.
